...
export class HorizonService {
  ...
  constructor() {
    ...
    console.log(
      `[Horizon] Service initialized for network: ${this.networkPassphrase.substring(0, 15)}...`
    );
  }
    ...
    if (this.circuitBreaker.isOpen) {
      ...
      if (now - this.circuitBreaker.lastFailure > this.circuitBreaker.cooldown) {
        ...
        throw new HorizonError(
          'Horizon service temporarily unavailable (Circuit Open)',
          503,
          'SERVICE_UNAVAILABLE'
        );
      }
  ...
  async getAccountDetails(
    address: string,
    retryOverride?: { retries?: number; delay?: number }
  ...
  async getTransactions(address: string, options: PaginationOptions = {}) {
    ...
    try {
      ...
      const response = (await this.withRetry(() =>
        query.call()
      )) as Horizon.ServerApi.CollectionPage<Horizon.ServerApi.TransactionRecord>;
  ...
  async getPayments(address: string, options: PaginationOptions = {}) {
    ...
    try {
      ...
      const response = (await this.withRetry(() =>
        query.call()
      )) as Horizon.ServerApi.CollectionPage<Horizon.ServerApi.PaymentOperationRecord>;
  ...
  private async withRetry<T>(fn: () => Promise<T>, retries = 3, delay = 1000): Promise<T> {
    ...
    try {
      ...
      if (isTransient && retries > 0) {
        ...
        console.warn(
          `[Horizon] Transient error detected (Status: ${status}, Code: ${err.code}). Retrying in ${Math.round(nextDelay)}ms... (${retries} retries left)`
        );
